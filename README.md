...
3. 登入後可進入受保護頁面，例如：
   ...
   - `/trades/portfolio`
4. 右上角可查看登入狀態與登出入口
5. 收藏、交易、持股與投資組合資料目前會依登入使用者隔離，但股票查詢與研究頁仍維持共用展示
...
- `watchlist`、`trades` 與 `portfolio` 相關資料已綁定到目前登入使用者
- 新增收藏、BUY、SELL 後，資料會自動歸屬到當前登入帳號
- 交易紀錄、持股總覽、已實現 / 未實現損益與投資組合摘要只計算目前登入使用者的資料
- 股票查詢、走勢圖、技術指標與研究頁仍維持共用查詢頁，不做 user-scoped 隔離
- 目前屬「最小資料隔離版」，尚未延伸到角色權限、共享資料或完整平台授權模型
...
- 首頁會提供精簡版 `Portfolio Workspace` preview，快速顯示現金、部位、市值與損益摘要
